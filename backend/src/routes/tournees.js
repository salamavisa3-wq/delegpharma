...
router.get('/districts/:id/professionnels', async (req, res) => {
  const rows = await all(`
    ...
    WHERE p.laboratoire_id = ? AND s.district_id = ?
...
router.get('/tournees', async (req, res) => {
  ...
  const clauses = ['t.laboratoire_id = ?'];
  const params = [req.user.laboratoire_id];
  if (req.user.role === 'delegue') { clauses.push('t.user_id = ?'); params.push(req.user.id); }
  if (statut) { clauses.push('t.statut = ?'); params.push(statut); }
...
router.post('/tournees', requireRole('delegue', 'manager', 'admin'), async (req, res) => {
  ...
  const r = await run(
    'INSERT INTO tournee (laboratoire_id, user_id, date, district_id, ps_list, statut) VALUES (?,?,?,?,?,?)',
...
router.post('/tournees/:id/faire', requireRole('delegue', 'manager', 'admin'), async (req, res) => {
  const t = await get('SELECT id, statut, user_id FROM tournee WHERE id = ? AND laboratoire_id = ?',
  ...
  await run('UPDATE tournee SET statut = ? WHERE id = ?', ['faite', req.params.id]);
...
router.post('/tournees/:id/annuler', requireRole('delegue', 'manager', 'admin'), async (req, res) => {
  ...
  if (!t) return res.status(404).json({ error: 'Tournée introuvable' });
  if (req.user.role === 'delegue' && t.user_id !== req.user.id) return res.status(403).json({ error: 'Non autorisé' });
  await run('UPDATE tournee SET statut = ? WHERE id = ?', ['annulee', req.params.id]);
  return res.json({ ok: true });
});

router.delete('/tournees/:id', requireRole('manager', 'admin'), async (req, res) => {
  await run('DELETE FROM tournee WHERE id = ? AND laboratoire_id = ?', [req.params.id, req.user.laboratoire_id]);
  return res.json({ ok: true });
});
