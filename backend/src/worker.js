...
// Frontend → Static Assets (edge) ; PDFs → KV (jamais de bucket public).
...
// PDFs générés par .github/workflows/pdf.yml (pdfkit, CPU illimité) → KV → servis ici.
app.use('/pdfs', async (req, res) => {
  const key = req.path.replace(/^\/+/, '');
  if (!key || key.includes('..')) return res.status(404).json({ error: 'PDF inconnu' });
  try {
    const { env } = await import('cloudflare:workers');
    const data = await env.PDFS.get(key, { type: 'arrayBuffer' });
    if (!data) return res.status(404).json({ error: 'PDF introuvable' });
