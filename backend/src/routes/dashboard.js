...
async function delegueView(req, labo) {
  ...
  const stats = await all('SELECT v.statut, COUNT(*) AS n FROM visite v WHERE v.laboratoire_id = ? AND v.user_id = ? GROUP BY v.statut',
    [labo, me]);
  const byStatus = Object.fromEntries(stats.map((s) => [s.statut, s.n]));
  const semaine = await get(
    `SELECT COUNT(*) AS n FROM visite v
     WHERE v.laboratoire_id = ? AND v.user_id = ? AND v.date >= date('now','-7 days')`, [labo, me]);
  const prochaines = await all(`
    ...
    WHERE v.laboratoire_id = ? AND v.user_id = ? AND v.prochaine_visite != '' AND v.prochaine_visite >= date('now')
  ...
  const tournees = await all(`
    ...
    WHERE t.laboratoire_id = ? AND t.user_id = ? AND t.statut = 'planifiee'
  ...
  const recentes = await all(`
    ...
    WHERE v.laboratoire_id = ? AND v.user_id = ?
...
async function managerView(req, labo) {
  ...
  const fileAttente = await all(`
    ...
    WHERE v.laboratoire_id = ? AND v.statut = 'soumis'
  ...
  const equipe = await all(`
    ...
    FROM users u WHERE u.laboratoire_id = ? AND u.role = 'delegue'
    ORDER BY u.nom`, [labo]);
  const parRegion = await all(PAR_REGION, [labo]);
  const tournees = await get(`
    SELECT (SELECT COUNT(*) FROM tournee t WHERE t.laboratoire_id = ? AND t.statut = 'planifiee') AS planifiees,
           (SELECT COUNT(*) FROM tournee t WHERE t.laboratoire_id = ? AND t.statut = 'faite') AS faites`, [labo, labo]);
...
async function laboView(req, labo) {
  ...
  const global = await get(`
    SELECT
      (SELECT COUNT(*) FROM visite WHERE laboratoire_id = ? AND statut = 'soumis') AS soumises,
      (SELECT COUNT(*) FROM visite WHERE laboratoire_id = ? AND statut = 'valide') AS validees,
      (SELECT COUNT(*) FROM visite WHERE laboratoire_id = ? AND statut = 'refuse') AS refusees,
      (SELECT COUNT(*) FROM professionnel WHERE laboratoire_id = ?) AS ps,
      (SELECT COUNT(*) FROM users WHERE laboratoire_id = ? AND role = 'delegue') AS delegues`,
  ...
  const parRegion = await all(`
    ...
    LEFT JOIN structure s ON s.id = v.structure_id
    LEFT JOIN region r ON r.id = s.region_id
    WHERE v.laboratoire_id = ? AND v.statut IN ('soumis','valide')
...
async function adminView(req, labo) {
  ...
  const users = await all('SELECT role, COUNT(*) AS n FROM users WHERE laboratoire_id = ? GROUP BY role', [labo]);
  const ps = await get('SELECT COUNT(*) AS n FROM professionnel WHERE laboratoire_id = ?', [labo]);
  const structures = await get('SELECT COUNT(*) AS n FROM structure WHERE laboratoire_id = ?', [labo]);
